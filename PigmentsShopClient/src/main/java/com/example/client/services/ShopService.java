...
public class ShopService {
    ...
    public static void openShop(){
        ...
        HttpEntity<Double> response = restTemplate.exchange(link,HttpMethod.GET,null,Double.class);
        shop.setPillow(response.getBody());
    ...
    private static void Purchase(Order order, String ans, Cashier cashier) throws Exception {
        ...
        for (int i = 0; i < colors.size(); i++) {
            colors.get(i).reduceWeight(order.countPigment(i));
    ...
    public static void closeShop(){
        ExpensesService.addToPillow(cashier.getCurrentBalance(),ExpensesService.buyPigments(colors),shop);
        ...
        HttpEntity<String> response = restTemplate.exchange(componentsBuilder.toUriString(), HttpMethod.PUT,null,String.class);
        System.out.println(response.getBody());


        link = "http://localhost:8080/pigments/colors";
        componentsBuilder = UriComponentsBuilder.fromHttpUrl(link)
                .queryParam("cyanWeight",colors.get(0).getWeight())
                .queryParam("magentaWeight",colors.get(1).getWeight())
                .queryParam("yellowWeight",colors.get(2).getWeight())
                .queryParam("blackWeight",colors.get(3).getWeight())
                .queryParam("whiteWeight",colors.get(4).getWeight());

        response = restTemplate.exchange(componentsBuilder.toUriString(), HttpMethod.PUT,null,String.class);
        System.out.println(response.getBody());
